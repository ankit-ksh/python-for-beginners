---
title: Videos
---

# Videos

- CS50’s Introduction to Programming with Python (CS50P)
	- https://www.youtube.com/playlist?list=PLhQjrBD2T3815jAA4c8CIOiZ7gQY1D2-o
	- University‑level course, excellent explanations and problem sets.

- MIT 6.0001 Introduction to Computer Science and Programming in Python
	- https://www.youtube.com/playlist?list=PLUl4u3cNGP63WbdFxL8giv4yhgdMGaZNA
	- Foundational CS with Python; pairs well with Guttag’s book.

- Python for Beginners – Full Course (freeCodeCamp)
	- https://www.youtube.com/watch?v=rfscVS0vtbw
	- Popular beginner‑friendly crash course with exercises.

- Corey Schafer – Python Tutorials (Playlist)
	- https://www.youtube.com/playlist?list=PL-osiE80TeTsqhIuOqKhwlXsIBIdSeYtc
	- Clear, practical tutorials on core topics and best practices.

- Sentdex – Python 3 Basics (Playlist)
	- https://www.youtube.com/playlist?list=PLQVvvaa0QuDfKTOs3Keq_kaG2P55YRn5v
	- Beginner series with many follow‑ups (data, ML, web dev).

